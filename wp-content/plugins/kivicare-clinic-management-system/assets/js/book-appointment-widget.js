        ...
        */
                    ...
                    if (response.data.status !== undefined && response.data.status === true) {
                        ...
                        if ([0, 1, 2, 3, 4, 5, 6].every(r => days.includes(r))) {
                            ...
                            flatpickr(".iq-inline-datepicker", {
                                ...
                                locale: window.data_value.full_calendar,
        ...
        function checkChildWindow() {
            ...
            if (child.closed && payment_status === '') {
                ...
                if (appointment_id !== '') {
                    axios.get(window.data_value.ajax_url + '?action=ajax_get&route_name=appointment_delete&id=' + appointment_id)
                        .then((response) => {
                        })
                        .catch((error) => {
                            console.log(error);
...
function kivicarePrintContent(id) {
    axios.get(window.data_value.ajax_url + '?action=ajax_get&route_name=get_appointment_print&id=' + id + '&calendar_enable=yes')
        .then((response) => {
            if (response.data.status !== undefined && response.data.status === true) {
