        ...
        if ($value === '') {
            ...
            if (is_array($value)) {
                ...
                if (is_object($value)) {
                    ...
                    $filterParameters[$key] = sanitize_email($value);
...
?>
    ...
    function kcCheckPermission($permission_name)
    {
        ...
        if (!empty($user_kc_roles)) {
            $permissions =  $kc_roles[array_pop(array_reverse($user_kc_roles))]()->toArray();
        }
    ...
    function kcEmailContentKeyReplace($content, $data)
    {
        ...
        $keys = [
            ...
            '{{appointment_date}}' => isset($data['appointment_date']) ? esc_html(date(get_option('date_format'), strtotime($data['appointment_date']))) : '',
            ...
            '{{prescription}}' => isset($data['prescription']) ? wp_kses_post($data['prescription']) : '',
    ...
    function kvGetTimeSlots($data, $new_time_slot = "", $only_available_slots = false)
    {
        ...
        if (!empty($data['service']) && isKiviCareProActive()) {
            ...
            if (!empty($service_id)) {
                ...
                $service_duration = $wpdb->get_var("SELECT SUM(duration) FROM {$wpdb->prefix}kc_service_doctor_mapping WHERE service_id IN ({$service_id}) AND doctor_id={$data['doctor_id']}");
            }
        }
                ...
                if (!empty($appointment_data) && is_array($appointment_data) && count($appointment_data) > 0) {
                    ...
                    foreach ($appointment_data as $appoint) {
                        ...
                        if (empty($appointment_reminder_data_id)) {
                            $wpdb->insert($msg_reminder_table, [
    ...
    function kivicareWoocommercePaymentComplete($order_id, $type = 'woocommerce')
    {
        ...
        $get_service_query = "SELECT {$service_mapping_table}.telemed_service,{$service_category_table}.name FROM {$appointment_mapping_table}
                                LEFT JOIN {$service_mapping_table} 
                                  ON {$service_mapping_table}.service_id= {$appointment_mapping_table}.service_id
                                LEFT JOIN {$service_category_table} 
        ...
        if (in_array($type, ['paypal', 'razorpay'])) {
    ...
    ?>
        ...
        function kcAppendLanguageInHead()
        {
            if (isKiviCareProActive()) {
                $prefix = KIVI_CARE_PREFIX;
    ...
    <script>
        window.__kivicarelang = <?php echo json_encode($var); ?>
    </script>
<?php
        }
        ...
        {
            $data = get_option(KIVI_CARE_PREFIX . 'date_format', true);
...
<?php
        ...
        function kcDashboardSidebarArray($user_roles)
        {
            ...
            //     $option_data = get_option(KIVI_CARE_PREFIX . "{$role}_dashboard_sidebar_data");
        ...
        function kcDoctorSidebarArray()
        {
            ...
            $data = [
                ...
                [
                    'label' => isset($translate_lang['dashboard']['dashboard']) ? $translate_lang['dashboard']['dashboard'] : esc_html__('Patients', 'kc-lang'),
                    'type' => 'route',
                    'link' => 'patient',
                    'iconClass' => 'fas fa-hospital-user',
                    'routeClass' => 'patient',
