            ...
            <div dir='<?php echo esc_html($rtl_attr); ?>' id="kivicare-doctor-id-main">
                ...
                if ($setting['iq_kivivare_doctor_enable_filter'] == 'yes'){
                    ...
                    $service_list = $wpdb->get_results("SELECT ser.name, CONCAT(ser.name,'(',REPLACE(ser.type,'_',' '),')') AS service_name FROM {$wpdb->prefix}kc_services AS ser 
                        JOIN {$wpdb->prefix}kc_service_doctor_mapping AS ser_map ON ser_map.service_id = ser.id WHERE ser.status = 1 AND ser_map.status = 1");
                    ...
                    (function ($) {
                        ...
                        function kivicareDoctorFilter(){
                            ...
                            $('#kivicare_filter_button').unbind().submit(function (e) {
                                ...
                                if (service === '') {
                                    // $(this).find('select[name="service"]').prop('disabled', true)
                                }
                                if (search === '') {
                                    // $(this).find('input[name="search"]').prop('disabled', true)
                                }
                                if (speciality === '') {
                                    // $(this).find('select[name="speciality"]').prop('disabled', true)
                                }
                                if (speciality === '' && search === '' && service === ''){
                                    console.log('Please select any filter value')
                                    // return
                                }
                                kivicareGetDoctorlist();
