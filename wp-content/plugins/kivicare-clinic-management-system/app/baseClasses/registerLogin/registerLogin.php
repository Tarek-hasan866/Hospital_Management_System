...
$clinic = collect($wpdb->get_results("SELECT id,name FROM {$wpdb->prefix}kc_clinics WHERE id IN ({$clinic_id})"))->toArray();
                        ...
                        <div  id="kivicare-login">
                            ...
                            <div>
                                <a href="<?php echo esc_url(wp_lostpassword_url()); ?>" target="_blank"
                                   class="iq-color-secondary"><i><?php echo esc_html__('Forgot Password ?', 'kc-lang'); ?></i></a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        if (event.target.readyState === "complete") {
            ...
            (function ($) {
                ...
                const get = (route, data, frontEnd = false) => {
                    ...
                    if (frontEnd) {
                        window.ajaxurl = '<?php echo esc_url(admin_url('admin-ajax.php'));?>';
                        window.nonce = '<?php echo esc_js(wp_create_nonce('ajax_get'));?>';
                    }
                ...
                $(document).on('submit', '#kivicare-login-form', function (event) {
                    ...
                    post('login_new_user', result, true)
                        .then((response) => {
                            ...
                            if (response.data.status !== undefined && response.data.status === true) {
                                setTimeout(() => {
                                    location.href = '<?php echo esc_url(admin_url('admin.php?page=dashboard')); ?>'
                                }, 1000)
                            } else {
                                kcShowErrorMessage(response.data.message)
                            }
