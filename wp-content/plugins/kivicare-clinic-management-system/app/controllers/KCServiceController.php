...
class KCServiceController extends KCBase {
	...
	public function index() {
        ...
        $search_condition = $doctor_condition  = $clinic_condition = $clinic_join = $paginationCondition = " ";
		...
		if(($this->getDoctorRole() === $current_login_user_role) || (isset($request_data['doctor_id']) && !in_array($request_data['doctor_id'],[0,'0']))) {
            ...
            //doctor id
            $doctor_id = $this->getDoctorRole() === $current_login_user_role ? $current_login_user_id :  (int)$request_data['doctor_id'];

            //doctor query condition
            $doctor_condition = " AND {$service_doctor_mapping}.doctor_id = {$doctor_id} " ;

        }
        //check current login user role is clinic-admin or receptionist
        elseif(in_array($current_login_user_role,[$this->getClinicAdminRole(),$this->getReceptionistRole()])){
            //clinic join query
            ...
            if(isKiviCareProActive()){
                //get clinic id based on current login user role
                $clinic_id = $this->getReceptionistRole() === $current_login_user_role ? kcGetClinicIdOfReceptionist(): kcGetClinicIdOfClinicAdmin() ;
            }
            else{
                //get default clinic id
                $clinic_id = kcGetDefaultClinicId();
            }
            //clinic id query condition
        ...
        $full_service_name = $request_from_new_appointment_widget_and_service_first ? " {$service_table}.name,{$service_table}.type " : " {$service_table}.name,{$service_table}.type,{$service_doctor_mapping}.doctor_id ";
        ...
        if($request_from_new_appointment_widget && isKiviCareProActive()){
            ...
            if(!empty($request_data['clinic_id']) && !in_array($request_data['clinic_id'],['0',0]) ){
                ...
                $clinic_condition = " AND {$clinic_doctor_mapping}.clinic_id = {$request_data['clinic_id']}" ;
                $clinic_join = " JOIN  {$clinic_doctor_mapping} ON {$clinic_doctor_mapping}.doctor_id = {$service_doctor_mapping}.doctor_id";
            }
        }
        ...
        $query = "SELECT {$service_doctor_mapping}.*,{$service_doctor_mapping}.charges as service_base_price,
                  ...
                  {$users_table}.display_name AS doctor_name  FROM {$service_doctor_mapping}
                  ...
                  {$clinic_join}
                  WHERE 0 = 0 {$doctor_condition} {$clinic_condition} {$active_services} {$telemed_condition} {$search_condition} {$preselected_service_condition}
                 {$orderByCondition}" ;
        ...
        if($request_from_service_module){
            $total = $this->db->get_var( "SELECT count(*)  FROM {$service_doctor_mapping}
                  ...
                  JOIN {$users_table} ON {$users_table}.ID = {$service_doctor_mapping}.doctor_id
                  {$clinic_join}
                  WHERE 0 = 0 {$doctor_condition} {$clinic_condition} {$active_services} {$telemed_condition} {$search_condition} " );
	...
	public function save() {
        ...
        if(!empty($request_data['id']) && isset($request_data['doctor_id']['id'])){
            $doctor_id = [];
            $doctor_id[0] = $request_data['doctor_id']['id'];
        }

        //check if same service exists with same doctor
        $validationQuery = "SELECT map.id FROM {$doctor_service_table} AS map LEFT JOIN {$service_table} AS ser ON ser.id=map.service_id WHERE 0=0 ";
        //implode doctor id for query
        $implode_doctor_id = implode(',',$doctor_id);

        //check if service update or save
        if(!empty($request_data['id'])){
            $request_data['id'] = (int)$request_data['id'];
            $validationRow = $wpdb->get_var($validationQuery ." AND map.id = {$request_data['id']}  AND map.doctor_id IN ({$implode_doctor_id}) AND ser.type ='{$temp['type']}' AND ser.name='{$request_data['name']}'");
            if($validationRow == $request_data['id']){
                $validationRow = '';
            }
        }else{
            $validationRow = $wpdb->get_var($validationQuery." AND map.doctor_id IN ({$implode_doctor_id}) AND ser.type ='{$temp['type']}' AND ser.name='{$request_data['name']}'");
        }
            ...
            if ($service_id) {
                ...
                foreach ($doctor_id as $doctor){
                    ...
                    $service_mapping_data = [
                        ...
                        'charges'    => (int)$request_data['price'],
	...
	public function delete() {
		...
		try {
			...
			if ( $results ) {
				...
				throw new Exception( esc_html__('Failed to delete Service', 'kc-lang'), 400 );
			}
        ...
        foreach( $services as $key => $main_services){
            ...
            <div class="d-flex flex-column gap-1 pt-2">
                <?php if($showServicetypeStatus){
                    ?>
                    <h5 class="iq-color-secondary iq-letter-spacing-1 pl-1"><?php echo esc_html($key); ?></h5>
                    <?php
                } ?>
