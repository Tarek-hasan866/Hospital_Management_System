...
class KCGeneralSettingController extends KCBase
{
    ...
    public function saveClinicCurrency(){
        ...
        if(!empty($request_data['clinic_data'])){
            ...
            $results = $this->db->query("UPDATE {$this->db->prefix}kc_clinics SET extra='".json_encode($currency, JSON_UNESCAPED_UNICODE)."'");
            $status = true;
    ...
    public function KCResetPluginData() {
        ...
        echo json_encode(['status' => $status, 'reset_all' => $reset_all, "message" => $message]);
    }
