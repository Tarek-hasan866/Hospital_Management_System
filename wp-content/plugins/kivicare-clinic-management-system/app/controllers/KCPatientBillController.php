	...
	public function save() {
		...
		if ( !empty( $request_data['billItems'] ) && count( $request_data['billItems'] ) ) {
            ...
            foreach ( $request_data['billItems'] as $key => $bill_item ) {
				if ( $bill_item['item_id']['label'] != 'Telemed' && (int)$bill_item['item_id']['price'] === 0 ) {
	...
	public function delete() {
		...
		try {
			...
			if ( $results ) {
				...
				throw new Exception( esc_html__('Failed to delete Bill item.', 'kc-lang'), 400 );
			}
	...
	public function updateStatus() {
		...
		try {
			...
			if ( $results ) {
				...
				throw new Exception( esc_html__('Failed to update status', 'kc-lang'), 400 );
			}
