...
//				'status_code' => 403,
	...
	public function save() {
		...
		if (gettype($request_data['visit_type']) === 'array') {
            $telemed_service_in_appointment_service = collect($request_data['visit_type'])->map(function ($v)use($request_data){
                $temp_service_id = (int)$v['service_id'];
                return $this->db->get_var("SELECT telemed_service FROM {$this->db->prefix}kc_service_doctor_mapping WHERE service_id = {$temp_service_id} AND doctor_id=".(int)$request_data['doctor_id']['id']);
            })->toArray();
	...
	public function delete() {
		...
		try {
            ...
            do_action( 'kc_appointment_delete', $id );
        ...
        if ( !empty($filterData['status']) ) {
            if (isset($filterData['status']['value'])) {
                if($filterData['status']['value'] !== 'all'){
                    ...
                    $query .= " AND {$appointments_table}.status = {$filterData['status']['value']} ";
                }
...
//            $telemedPluginActive = isKiviCareTelemedActive();
            ...
            $appointments = $appCollection->map(function ($appointment) use ($current_date,$currency_prefix,$currency_postfix,$zoom_mappings,$is_patient_enable,$current_login_user_role,$googleMeetPluginActive,$proPluginActive,$enableAppointmentDescription) {
				...
				LEFT JOIN {$appointments_service_table} ON {$appointments_table}.id = {$appointments_service_table}.appointment_id JOIN {$service_table} 
				ON {$appointments_service_table}.service_id = {$service_table}.id JOIN {$service_doctor_table} ON {$service_doctor_table}.service_id ={$service_table}.id and {$service_doctor_table}.doctor_id={$appointment->doctor_id['id']} and {$appointments_service_table}.appointment_id = {$appointment->id}  WHERE 0 = 0";
