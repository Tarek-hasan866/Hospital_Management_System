    ...
    public function saveAppointment($request) {
        ...
        try {
            ...
            if(!is_user_logged_in()) {
                throw new Exception( esc_html__('Please Sign in to book appointment', 'kc-lang'), 401 );
            }

            $userObj = wp_get_current_user();

            if (!in_array($this->getPatientRole(),$userObj->roles)) {
                throw new Exception( esc_html__('User must be a Patient to book appointment', 'kc-lang'), 401 );
            }
