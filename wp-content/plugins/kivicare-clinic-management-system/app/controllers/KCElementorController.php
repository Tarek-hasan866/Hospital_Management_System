...
class KCElementorController extends KCBase{
    ...
    public function clinicWiseDoctor($clinic_id,$request_data){
        ...
        if(!empty($filter_data['service'])){
            ...
            $query = "SELECT ser_map.doctor_id FROM {$wpdb->prefix}kc_service_doctor_mapping AS ser_map 
                      JOIN {$wpdb->prefix}kc_services AS ser ON ser.id = ser_map.service_id WHERE ser.name lIKE '%".esc_sql(trim($filter_data['service']))."%'";
    ...
    public function clinicIndex(){
        ...
        <?php
        echo json_encode(['status' => true,'data' => ob_get_clean()]);
        die;
    }
    public function clinicListData($request_data) {
        ...
        if(!isKiviCareProActive()){
            ...
            if(!empty($filter_data['service'])){
                ...
                $service_query = "SELECT cl.id FROM {$clinic_table} AS cl LEFT JOIN {$wpdb->prefix}kc_doctor_clinic_mappings AS doc_map ON doc_map.clinic_id=cl.id
                                    LEFT JOIN {$wpdb->prefix}kc_service_doctor_mapping AS doc_ser ON doc_ser.doctor_id=doc_map.doctor_id 
                                    LEFT JOIN {$wpdb->prefix}kc_services AS  ser ON ser.id=doc_ser.service_id 
                                    WHERE ser.name LIKE '%{$service_query_value}%' ";

                $clinic_id_service_wise = collect($wpdb->get_results($service_query))->unique('id')->pluck('id')->toArray();
